...
def arg_parse():
    ...
    io_parser.add_argument('--graph_type', dest='graph_type',
                           help='Input context_centernode or doc_word or doc_word_sen for composition.')
    io_parser.add_argument("--label_idx", dest="label_idx", help="label type")
    io_parser.add_argument("--explain_nodes", dest="explain_nodes", help="Input nodes_idx which are need to explain.")
    ...
    parser.add_argument("--label_idx", dest="label_idx", help="label_idx")
    ...
    parser.set_defaults(
        ...
        datadir="data",
        dataset="im",
        graph_type="doc_word_sen",
        label_idx="1客户感受差",
        explain_nodes=range(640,656),
        opt="adam",
        opt_scheduler="none",
        cuda="0",
        lr=0.1,
...
def main():
    ...
    if prog_args.writer:
        path = os.path.join(prog_args.logdir, prog_args.graph_type,prog_args.label_idx,io_utils.gen_explainer_prefix(prog_args))
        ...
        if os.path.isdir(path) and prog_args.clean_log:
           ...
           shutil.rmtree(path)
        writer = SummaryWriter(path+ '-' + time.strftime('%m_%d_%H_%M', time.localtime()))
    else:
        writer = None
    ...
    if prog_args.explain_node is not None:
        ...
        if prog_args.multinode_class >= 0:
            ...
            # explain a set of nodes 维度确定的理由:取哪些点做研究 cg_dict['adj'].shape[1] model:exp,grad
            masked_adj = explainer.explain_nodes_gnn_stats(
                range(655), prog_args,model='exp'
            )
