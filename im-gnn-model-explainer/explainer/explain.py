...
class Explainer:
    ...
    def explain(
        ...
        if self.graph_mode:
            ...
            if pred_label.size==0:
                print(f"Node {node_idx} no neighbors")
                return np.array([])
            else:
        ...
        if model == "grad":
            ...
            if model == "exp":
                ...
                masked_feat = np.expand_dims(explainer.feat_mask.cpu().detach().numpy(), axis=1)
            else:
                adj_atts = nn.functional.sigmoid(adj_atts).squeeze()
                masked_adj = adj_atts.cpu().detach().numpy() * sub_adj.squeeze()
        parent_path = os.path.join(self.args.logdir, 'npy',model, self.args.graph_type,self.args.label_idx)
        os.makedirs(parent_path,exist_ok=True)
        fname = 'masked_adj_' + io_utils.gen_explainer_prefix(self.args) + (
                'node_idx_' + str(node_idx) + 'graph_idx_' + str(self.graph_idx) + '.npy')
        with open(os.path.join(parent_path, fname), 'wb') as outfile:
            np.save(outfile, np.asarray(masked_adj.copy()))
            print("Saved adjacency matrix to ", fname)
        fname_featmask = 'masked_featmask_' + io_utils.gen_explainer_prefix(self.args) + (
                'node_idx_'+str(node_idx)+'graph_idx_'+str(self.graph_idx)+'.npy')
        with open(os.path.join(parent_path, fname_featmask), 'wb') as outfile:
            np.save(outfile, np.asarray(masked_feat.copy()))
            print("Saved adjacency matrix to ", fname_featmask)
        fname_neighbors = 'masked_neighbors_' + io_utils.gen_explainer_prefix(self.args) + (
                'node_idx_' + str(node_idx) + 'graph_idx_' + str(self.graph_idx) + '.npy')
        with open(os.path.join(parent_path, fname_neighbors), 'wb') as outfile:
            np.save(outfile, np.asarray(neighbors.copy()))
            print("Saved neighbors to ", fname)
        return masked_adj
    ...
    def explain_nodes_gnn_stats(self, node_indices, args, graph_idx=0, model="exp"):
        ...
        '''
        # pdb.set_trace() 下面是计算各个node_idx的子图准确率指标
        graphs = []
        feats = []
        adjs = []
        pred_all = []
    ...
    def make_pred_real(self, adj, start):
        ...
        if self.args.dataset == "syn1" or self.args.dataset == "syn2":
            ...
            pred = adj[np.triu(adj) > 0]  # 上三角函数 k=0默认
            real = adj.copy()
...
class ExplainModule(nn.Module):
    ...
    def forward(self, node_idx, unconstrained=False, mask_features=True, marginalize=False):
        ...
        if unconstrained:
            ...
            self.masked_adj = self._masked_adj() # 无连接处+对角线 mask
            if mask_features:
                feat_mask = (
