...
def train_node_classifier(G, labels, model, args, writer=None):
    ...
    if 'noshuffle' in args.graph_type:
        np.random.shuffle(idx)
        num_train = int(num_nodes * args.train_ratio)
        train_idx = idx[:num_train]
        test_idx = idx[num_train:]
    else:
        test_nodeid_range = [range(0, 596), range(1193, 1337), range(1480, 1522), range(1563, 1618), range(1672, 1705),
                             range(1737, 1757), range(1777, 1795)]
        train_nodeid_range = [range(597, 1192), range(1338, 1479), range(1523, 1562), range(1619, 1671),
                              range(1706, 1736), range(1758, 1776), range(1796, 1812)]

        train_idx, test_idx = [], []
        for tr, ts in zip(train_nodeid_range, test_nodeid_range):
            train_idx.extend(list(tr))
            test_idx.extend(list(ts))
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.gpu:
            ...
            loss = model.loss(ypred_train, labels_train)  # (torch.Size([1, 71, 4]), torch.Size([1, 71]))
        loss.backward()
        nn.utils.clip_grad_norm(model.parameters(), args.clip)
...
#
#############################
def im_task(args, writer=None):
    # data
    G, labels, name = gengraph.gen_im(
        args
    )
...
def main():
    ...
    if prog_args.pkl_fname is not None:
        pkl_task(prog_args)
    if prog_args.dataset is not None:
        if prog_args.dataset == "syn1":
            syn_task1(prog_args, writer=writer)
        elif prog_args.dataset == "im":
            im_task(prog_args, writer=writer)
